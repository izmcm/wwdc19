...
public class SecondLevelScene: SKScene {
    ...
    public func drawBoard() {
        ...
        cross.position = CGPoint(x: 1.5*cross.frame.size.width/2, y: screenHeight/2)
        cross.zRotation = .pi
        self.addChild(cross)
        ...
        blu.position = CGPoint(x: cross.position.x + cross.frame.size.width/2 + blu.frame.size.width, y: screenHeight/1.9)
        blu.zPosition = 1000
        self.addChild(blu)
        
        let school = SKSpriteNode(imageNamed: "school")
        school.size = CGSize(width: 1.2*screenWidth/4, height: screenWidth/4)
        school.position = CGPoint(x: cross.position.x + cross.frame.size.width/2 + 1.2*school.frame.size.width/2, y: screenHeight - screenHeight/10 - 0.7*school.frame.size.height)
        self.addChild(school)
        
        /************************** INSTRUCTIONS **************************/
        let strBlu = NSAttributedString(string:"Blu needs to go to school", attributes:
            [NSAttributedString.Key.foregroundColor: #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1),
             NSAttributedString.Key.font: UIFont.systemFont(ofSize: FONT_SIZE, weight: .medium),
             NSAttributedString.Key.paragraphStyle: paragraphCenter])
        ...
        upBtn.position = CGPoint(x: screenWidth/2 + upBtn.frame.width/2, y: line.position.y - upBtn.frame.size.height)
        self.addChild(upBtn)
        ...
        rightBtn.position = CGPoint(x: upBtn.position.x - upBtn.frame.size.width - upBtn.frame.size.width/4, y: line.position.y - rightBtn.frame.size.height)
        ...
        downBtn.position = CGPoint(x: rightBtn.position.x - rightBtn.frame.size.width - rightBtn.frame.size.width/4, y: line.position.y - downBtn.frame.size.height)
        ...
        leftBtn.position = CGPoint(x: upBtn.position.x + upBtn.frame.size.width + upBtn.frame.size.width/4, y: line.position.y - leftBtn.frame.size.height)
    ...
    func runBtnClick() {
        if codeAns.count <= LIMIT_CODE {
            ...
            if codeAns == [RIGHT_ACTION, UP_ACTION] || codeAns == [UP_ACTION, RIGHT_ACTION] {
                stars += 1
